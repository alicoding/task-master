#!/usr/bin/env tsx
/**
 * JS to TS Import Extension Converter
 * 
 * This script converts .js extensions in imports back to .ts extensions
 * to maintain strict TypeScript-only codebase
 * 
 * It handles:
...
console.log(`
${colors.cyan}${colors.bold}TypeScript-Only Import Fixer${colors.reset}
...
function verifyNoJsImports(sourceFile: SourceFile): boolean {
  ...
  // Skip this file itself and specific files with intentional .js references to avoid false positives
  if (filePath.endsWith('fix-js-to-ts-imports.ts') || 
      filePath.endsWith('fix-js-imports.ts') ||
      filePath.includes('examples/add-task-examples.ts')) {
    return false;
  }
