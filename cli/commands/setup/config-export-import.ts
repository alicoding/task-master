...
function envToConfiguration(env: Record<string, string>): ConfigurationSchema {
  ...
  if (env.DB_PATH) {
    if (!config.sections.database) {
      ...
      config.sections.database.path = env.DB_PATH;
      config.sections.database.enableMigrations = env.DB_ENABLE_MIGRATIONS === 'true';
      config.sections.database.enableBackups = env.DB_ENABLE_BACKUPS === 'true';
    }

    if (env.DB_BACKUP_INTERVAL && config.sections.database) {
      config.sections.database.backupInterval = parseInt(env.DB_BACKUP_INTERVAL);
    }

    if (env.DEBUG_SQL && config.sections.database) {
...
function detectSensitiveInfo(config: ConfigurationSchema): string[] {
  ...
  Object.keys(config.sections.general).forEach(key => {
    ...
    // Make sure general section exists before checking
    if (!config.sections.general) {
      config.sections.general = {};
    }
    const value = key in config.sections.general ? config.sections.general[key] : null;
    if (typeof value === 'string' &&
        value.length > 20 &&
  ...
  if (specificSections && specificSections.length > 0) {
    ...
    specificSections.forEach(section => {
      ...
      if (config.sections[sectionKey]) {
        newSections[section] = config.sections[sectionKey];
      }
    });
  ...
  if (sensitiveInfo.length > 0 && !includeSecrets) {
    ...
    if (!includeSecretsPrompt) {
      sensitiveInfo.forEach(key => {
        ...
        // Now it's safe to check and delete
        if (config.sections.general[key]) {
    ...
    p.note(
      [
        ...
        sensitiveInfo.length > 0 
          ? `Sensitive information: ${includeSecrets ? chalk.yellow('Included') : chalk.green('Excluded')}`
...
export async function importConfiguration(
  ...
  try {
    ...
    if (validateNow) {
      ...
      const { runConfigurationValidation } = await import('./config-validator');
