...
import { setupAiConfiguration } from './ai-config';
import { setupDbConfiguration } from './db-config';
import { setupNewProject } from './project-init';
import { runConfigurationValidation } from './config-validator';
import { runConnectionTest } from './connection-tester';
  ...
  setupCommand.action(async (options) => {
    ...
    try {
      if (options.export !== undefined) {
        ...
        const { exportConfiguration } = await import('./config-export-import');

        // Parse sections if provided
        const sections = typeof options.export === 'string'
          ? options.export.split(',').map((s: string) => s.trim())
          : undefined;

        await exportConfiguration(options.force, sections);
      } else if (options.import !== undefined) {
        // Import configuration
        const { importConfiguration } = await import('./config-export-import');

        // Parse sections if provided
        const sections = typeof options.import === 'string'
          ? options.import.split(',').map((s: string) => s.trim())
          : undefined;
        ...
        const message = `Welcome to the ${chalk.blue('Task Master')} setup wizard. This will help you configure your installation.`;
        ...
        if (setupMode === 'init') {
          ...
          // Show help message
          p.log.info(chalk.gray('If set to No, you will be prompted for each conflict'));
