...
export function displayDuplicateGroups(
  ...
  console.log(colorize(`\n🔍 Found ${duplicateGroups.length} potential duplicate groups`, 'green', 'bold'));
  ...
  for (let i = 0; i < limitedGroups.length; i++) {
    ...
    console.log(
      colorize(`┌─ Group ${groupNumber} `, groupColor, 'bold') + 
    ...
    group.tasks.forEach((task, taskIndex) => {
      ...
      console.log(
        ...
        colorize(` • Tags: `, 'gray') + (task.tags.length > 0 ? 
      ...
      console.log(
        ...
        colorize(' • Updated: ', 'gray') + updatedDate
      );
    });
    ...
    if (group.maxSimilarity >= 0.8) {
      console.log(
        colorize('│ ', groupColor) + 
        colorize('⚠️  Recommendation: ', 'red', 'bold') + 
        colorize('These tasks should be merged', 'red')
      );
    } else if (group.maxSimilarity >= 0.6) {
      console.log(
        colorize('│ ', groupColor) + 
        colorize('ℹ️  Recommendation: ', 'yellow', 'bold') + 
...
export function displayDetailedGroupView(
  ...
  console.log(colorize(`\n● TASKS OVERVIEW (${selectedGroup.tasks.length} tasks)`, 'blue', 'bold'));
  ...
  console.log(colorize('\n● DETAILED TASK INFORMATION', 'blue', 'bold'));
    ...
    if (task.metadata) {
      ...
      if (Object.keys(cleanMetadata).length > 0) {
        ...
        for (const [key, value] of Object.entries(cleanMetadata)) {
          console.log(`  │   ${colorize(key, 'gray')}: ${JSON.stringify(value)}`);
        }
      }
    }
...
export function displayInteractiveHelp(colorize: ColorizeFunction) {
  console.log(colorize('\n┌' + '─'.repeat(60) + '┐', 'cyan'));
  console.log(colorize('│ DEDUPLICATION INTERACTIVE MODE', 'cyan', 'bold') + colorize(' '.repeat(34) + '│', 'cyan'));
...
export function displayTaskComparison(
  ...
  // Recommendation
  if (percentage >= 90) {
    console.log(colorize('\n⚠️  RECOMMENDATION: These tasks are almost certainly duplicates and should be merged.', 'red', 'bold'));
  } else if (percentage >= 80) {
    console.log(colorize('\n⚠️  RECOMMENDATION: These tasks are likely duplicates and should be merged.', 'red'));
  } else if (percentage >= 70) {
    console.log(colorize('\nℹ️  RECOMMENDATION: These tasks are very similar and should be reviewed for potential duplication.', 'magenta'));
  } else if (percentage >= 50) {
    console.log(colorize('\nℹ️  RECOMMENDATION: These tasks are moderately similar and may be related.', 'yellow'));
  } else {
    console.log(colorize('\nℹ️  These tasks show only minor similarity.', 'green'));
  }
}
...
export function displayTaskDetail(
  ...
  // Header
  console.log(colorize(`\n┌${'─'.repeat(78)}┐`, 'yellow'));
  console.log(colorize(`│ TASK DETAILS - [${taskIndex + 1}] ${task.id} ${' '.repeat(78 - 16 - task.id.length - String(taskIndex + 1).length)}│`, 'yellow', 'bold'));
  ...
  // Similarity section
  console.log(colorize('\n● SIMILARITY TO OTHER TASKS', 'blue', 'bold'));
