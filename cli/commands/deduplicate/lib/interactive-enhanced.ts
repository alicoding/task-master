...
export async function runInteractiveMode(limitedGroups: DuplicateGroup[], repo: TaskRepository, colorize: ColorizeFunction) {
    ...
    while (running) {
        ...
        let prompt = colorize('\nEnter command', asChalkColor((asChalkColor(('cyan')))));
        if (currentGroup !== null) {
            prompt += colorize(` (current group: ${currentGroup + 1})`, asChalkColor((asChalkColor(('blue')))));
        }
        prompt += colorize(': ', asChalkColor((asChalkColor(('cyan')))));
        ...
        if (command === 'r') {
            console.log(colorize('Refreshing duplicate list...', asChalkColor((asChalkColor(('blue'))))));
            console.log(colorize('(Note: This would reload tasks and recalculate duplicates in a real implementation)', asChalkColor((asChalkColor(('gray'))))));
            continue;
        }
        ...
        if (command.startsWith('c ')) {
            ...
            if (currentGroup === null) {
                console.log(colorize('Please select a group first using "v <group>" command.', asChalkColor((asChalkColor(('yellow'))))));
                continue;
            }
            ...
            if (parts.length === 2) {
                ...
                if (isNaN(task1) || isNaN(task2) ||
                    ...
                    task1 === task2) {
                    console.log(colorize(`Invalid task numbers. Must be different and between 1 and ${limitedGroups[currentGroup].tasks.length}.`, asChalkColor((asChalkColor(('red'))))));
                    continue;
                }
                displayTaskComparison(task1, task2, limitedGroups[currentGroup], colorize);
            }
            else {
                console.log(colorize('Invalid compare command. Use "c <task1> <task2>" format.', asChalkColor((asChalkColor(('red'))))));
            }
            continue;
        }
        ...
        if (command.startsWith('s ')) {
            ...
            if (isNaN(taskNum) || taskNum < 1 || taskNum > limitedGroups[currentGroup].tasks.length) {
                console.log(colorize(`Invalid task number. Must be between 1 and ${limitedGroups[currentGroup].tasks.length}.`, asChalkColor((asChalkColor(('red'))))));
                continue;
            }
        ...
        console.log(colorize(`Unknown command: "${command}". Type "h" for help.`, asChalkColor((asChalkColor(('red'))))));
    }
}
/**
...
export async function runAutoMergeSuggestions(highSimilarityGroups: DuplicateGroup[], repo: TaskRepository, colorize: ColorizeFunction) {
    ...
    console.log(colorize(`\n🔍 Auto-merge suggestions for ${highSimilarityGroups.length} groups`, asChalkColor((asChalkColor(('blue')))), 'bold'));
    console.log(colorize(`   Processing groups with 80%+ similarity...`, asChalkColor((asChalkColor(('gray'))))));
    console.log(colorize(`\n┌${'─'.repeat(78)}┐`, asChalkColor((asChalkColor(('green'))))));
    console.log(colorize(`│ AUTO-MERGE WIZARD ${' '.repeat(61)}│`, asChalkColor((asChalkColor(('green')))), 'bold'));
    ...
    for (let i = 0; i < highSimilarityGroups.length; i++) {
        ...
        if (i < highSimilarityGroups.length - 1) {
            console.log(colorize(`\n${'─'.repeat(78)}`, asChalkColor((asChalkColor(('gray'))))));
        }
    }
    // Display summary
    console.log(colorize(`\n┌${'─'.repeat(78)}┐`, asChalkColor((asChalkColor(('green'))))));
    console.log(colorize(`│ AUTO-MERGE SUMMARY ${' '.repeat(60)}│`, asChalkColor((asChalkColor(('green')))), 'bold'));
    console.log(colorize(`└${'─'.repeat(78)}┘`, asChalkColor((asChalkColor(('green'))))));
    console.log(colorize(`\n● RESULTS OVERVIEW`, asChalkColor((asChalkColor(('blue')))), 'bold'));
    console.log(`  Total groups processed: ${stats.processed}`);
    console.log(`  Groups merged: ${colorize(stats.merged.toString(), asChalkColor((asChalkColor(('green')))))}`);
    console.log(`  Groups skipped: ${colorize(stats.skipped.toString(), asChalkColor((asChalkColor(('yellow')))))}`);
    if (stats.merged > 0) {
        console.log(colorize(`\n● MERGE ACTIONS`, asChalkColor((asChalkColor(('blue')))), 'bold'));
        console.log(`  Tasks deleted: ${colorize(stats.tasksDeleted.toString(), asChalkColor((asChalkColor(('red')))))}`);
        console.log(`  Tasks marked as duplicates: ${colorize(stats.tasksMarkedAsDuplicate.toString(), asChalkColor((asChalkColor(('yellow')))))}`);
    }
    console.log(colorize(`\n✅ Auto-merge process completed`, asChalkColor((asChalkColor(('green')))), 'bold'));
}
