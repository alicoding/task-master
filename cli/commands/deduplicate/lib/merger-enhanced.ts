...
export async function handleMerge(group: DuplicateGroup, repo: TaskRepository, colorize: ColorizeFunction): Promise<MergeResult> {
    ...
    console.log(colorize(`│ TASK MERGE WIZARD ${' '.repeat(62)}│`, asChalkColor('green'), 'bold'));
    console.log(colorize(`└${'─'.repeat(78)}┘`, asChalkColor('green')));
    console.log(colorize('\n● TASKS TO MERGE', asChalkColor('blue'), 'bold'));
    ...
    for (let i = 0; i < group.tasks.length; i++) {
        ...
        console.log(colorize(`  [${taskNumber}] `, asChalkColor('blue'), 'bold') +
        ...
        console.log(`     Status: ${colorizeStatus(task.status, colorize)}, ` +
            ...
            `Tags: ${formatTags(task.tags, asChalkColor('cyan'))}`);
        console.log(`     Updated: ${new Date(task.updatedAt).toLocaleString()}`);
    }
    // Step 1: Select primary task to keep
    console.log(colorize('\n● STEP 1: SELECT PRIMARY TASK', asChalkColor('blue'), 'bold'));
    ...
    console.log(colorize('\n● STEP 2: SELECT TASKS TO MERGE', asChalkColor('blue'), 'bold'));
    ...
    const secondaryTasksInput = await new Promise<string>(resolve => {
        rl2.question(colorize('  Tasks to merge (comma-separated, "all", or "q" to cancel): ', asChalkColor('cyan')), resolve);
    });
    rl2.close();
    // Handle cancellation
    ...
    console.log(colorize('\n● MERGE PREVIEW', asChalkColor('blue'), 'bold'));
    console.log(colorize('  Primary Task:', asChalkColor('green'), 'bold'));
    ...
    console.log(colorize('  Secondary Tasks:', asChalkColor('yellow'), 'bold'));
    ...
    console.log(colorize('  Result After Merge:', asChalkColor('blue'), 'bold'));
    ...
    console.log(`    Tags: ${Array.from(allTags).map((tag: string) => colorize(tag, asChalkColor('cyan'))).join(', ')}`);
    console.log(`    Metadata will include merge history and details`);
    // Confirm merge
    console.log(colorize('\n● STEP 3: CONFIRM MERGE', asChalkColor('blue'), 'bold'));
    ...
    // Execute merge
    console.log(colorize('\n● MERGING TASKS', asChalkColor('blue'), 'bold'));
    ...
    try {
        ...
        console.log(colorize('\n● STEP 4: SECONDARY TASKS', asChalkColor('blue'), 'bold'));
        ...
        // Summary
        console.log(colorize('\n✅ MERGE COMPLETED SUCCESSFULLY', asChalkColor('green'), 'bold'));
        ...
        console.log(colorize('\n✖ MERGE FAILED', asChalkColor('red'), 'bold'));
...
export async function suggestMerge(group: DuplicateGroup, repo: TaskRepository, colorize: ColorizeFunction, groupNumber: number, totalGroups: number): Promise<MergeResult> {
    ...
    console.log(colorize(`\n● GROUP ${groupNumber}/${totalGroups} - ${similarity}% SIMILARITY`, asChalkColor('blue'), 'bold'));
    // Display tasks
    for (let i = 0; i < group.tasks.length; i++) {
        const task = group.tasks[i];
        const isNewest = task.id === newestTask.id;
        console.log((isNewest ? colorize('  ► ', asChalkColor('green'), 'bold') : '    ') +
    ...
    console.log(colorize('\n  Suggestion:', asChalkColor('yellow'), 'bold'));
    console.log(`    Merge all tasks into ${colorize(newestTask.id, asChalkColor('green'))}: ${newestTask.title}`);
    console.log(`    Combined tags: ${Array.from(allTags).map((tag: string) => colorize(tag, asChalkColor('cyan'))).join(', ') || colorize('none', asChalkColor('gray'))}`);
