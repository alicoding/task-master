...
export async function handleSearchCommand(options: SearchCommandOptions): Promise<void> {
    try {
        ...
        try {
            ...
            const { tasks, extractedInfo } = await executeSearch(repo, nlpService, filters, options.query, options.explain, useFuzzy);
            ...
            displaySearchResults(sortedTasks, format, options.explain ? extractedInfo : undefined, options.query, useFuzzy, options.color);
        }
        finally {
 ...
 */
export async function executeSearch(repo: TaskRepository, nlpService: NlpService, filters: SearchFilters, query?: string, explain?: boolean, useFuzzy?: boolean): Promise<{
    tasks: Task[];
...
export function sortSearchResults(tasks: Task[], sortField?: string): Task[] {
    ...
    return [...tasks].sort((a, b) => {
        ...
        if (sortField === 'readiness')
            return String(a.readiness || '').localeCompare(String(b.readiness || ''));
 ...
 */
export function displaySearchResults(tasks: Task[], format: OutputFormat, extractedInfo?: ExtractedSearchFilters, query?: string, useFuzzy?: boolean, useColor?: boolean): void {
    if (format === 'json') {
 ...
 */
export async function performSimilaritySearch(repo: TaskRepository, nlpService: NlpService, options: SearchCommandOptions): Promise<void> {
    const format = options.format as OutputFormat || 'text';
