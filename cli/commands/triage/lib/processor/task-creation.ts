 ...
 */
export async function createNewTask(taskData: TriageTask, repo: TaskRepository, results: TriageResults, options: ProcessingOptions) {
    ...
    if (!dryRun) {
        ...
        if (!jsonOutput) {
            ...
            const properties = [];
            if (taskData.status)
                properties.push(`status: ${taskData.status}`);
            if (taskData.readiness)
                properties.push(`readiness: ${taskData.readiness}`);
