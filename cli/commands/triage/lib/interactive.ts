...
export async function runInteractiveMode(repo: TaskRepository, nlpService: NlpService, results: TriageResults, options: ProcessingOptions) {
    ...
    if (openTasks.length === 0) {
        if (jsonOutput) {
            ...
            console.log(colorize('No open tasks to triage.', asChalkColor((asChalkColor(('yellow'))))));
        }
        return;
    }
    if (!jsonOutput) {
        console.log(colorize(`Found ${openTasks.length} open tasks to triage.`, asChalkColor((asChalkColor(('green'))))));
    ...
    for (let i = 0; i < openTasks.length; i++) {
        ...
        if (!jsonOutput) {
            ...
            // Handle user actions
            if (action === 'q') {
                console.log(colorize('Exiting interactive mode.', asChalkColor((asChalkColor(('yellow'))))));
                break;
            }
            if (action === 's') {
...
async function displayTaskDetails(task: any, index: number, total: number, allTasks: any[], colorize: (text: string, color?: ChalkColor, style?: ChalkStyle) => string) {
    ...
    console.log(colorize(`ID: ${task.id}`, asChalkColor((asChalkColor(('blue'))))));
    ...
    if (task.parentId) {
        ...
        if (parentTask) {
            console.log(`Parent: ${colorize(parentTask.id, asChalkColor((asChalkColor(('yellow')))))}: ${parentTask.title}`);
        }
    }
    // Find child tasks
    const childTasks = allTasks.filter(t => t.parentId === task.id);
    if (childTasks.length > 0) {
        console.log(colorize(`\nChild Tasks (${childTasks.length}):`, asChalkColor((asChalkColor(('magenta'))))));
        childTasks.forEach(child => {
            console.log(`  ${colorize(child.id, asChalkColor((asChalkColor(('gray')))))}: ${child.title} [${child.status}]`);
        });
    }
}
/**
...
async function displaySimilarTasks(filteredTasks: any[], colorize: (text: string, color?: ChalkColor, style?: ChalkStyle) => string) {
    console.log(colorize(`\nSimilar Tasks Found (${filteredTasks.length}):`, asChalkColor((asChalkColor(('yellow'))))));
    filteredTasks.forEach((t, idx) => {
        ...
        console.log(`  ${colorize(`[${idx + 1}]`, asChalkColor((asChalkColor(('blue')))))} ${t.id}: ${t.title}`);
...
async function promptForAction(filteredTasks: any[], colorize: (text: string, color?: ChalkColor, style?: ChalkStyle) => string): Promise<string> {
    ...
    console.log(colorize('  d', asChalkColor((asChalkColor(('red'))))) + ') ' + colorize('Done', asChalkColor((asChalkColor(('white'))))) + ' - Mark task as completed');
    console.log(colorize('  t', asChalkColor((asChalkColor(('green'))))) + ') ' + colorize('Tags', asChalkColor((asChalkColor(('white'))))) + ' - Add/remove tags');
    if (filteredTasks.length > 0) {
        console.log(colorize('  m', asChalkColor((asChalkColor(('magenta'))))) + ') ' + colorize('Merge', asChalkColor((asChalkColor(('white'))))) + ' - Merge with a similar task');
    }
    console.log(colorize('  s', asChalkColor((asChalkColor(('gray'))))) + ') ' + colorize('Skip', asChalkColor((asChalkColor(('white'))))) + ' - Skip this task');
    console.log(colorize('  q', asChalkColor((asChalkColor(('red'))))) + ') ' + colorize('Quit', asChalkColor((asChalkColor(('white'))))) + ' - Exit interactive mode');
    ...
    const action = await new Promise<string>(resolve => {
        rl.question(colorize('\nChoose an action: ', asChalkColor((asChalkColor(('cyan'))))), resolve);
    });
    rl.close();
...
async function handleUpdateAction(task: any, repo: TaskRepository, results: TriageResults, options: ProcessingOptions) {
    ...
    if (dryRun) {
        console.log(colorize('Would update task (dry run).', asChalkColor((asChalkColor(('yellow'))))));
    ...
    console.log(colorize('\nCurrent status: ', asChalkColor((asChalkColor(('blue'))))) + colorizeStatus(task.status, colorize));
    const newStatusInput = await new Promise<string>(resolve => {
        rl2.question(colorize('New status [todo, in-progress, done] (leave empty to keep current): ', asChalkColor((asChalkColor(('cyan'))))), resolve);
    });
    rl2.close();
    ...
    if (newStatusInput) {
        ...
        if (['todo', 'in-progress', 'done'].includes(newStatusInput)) {
            ...
            console.log(colorize('Invalid status. Using current value.', asChalkColor((asChalkColor(('red'))))));
        }
    }
    // Get new readiness
    ...
    console.log(colorize('Current readiness: ', asChalkColor((asChalkColor(('blue'))))) + colorizeReadiness(task.readiness, colorize));
    const newReadinessInput = await new Promise<string>(resolve => {
        rl3.question(colorize('New readiness [draft, ready, blocked] (leave empty to keep current): ', asChalkColor((asChalkColor(('cyan'))))), resolve);
    });
    rl3.close();
    ...
    if (newReadinessInput) {
        ...
        if (['draft', 'ready', 'blocked'].includes(newReadinessInput)) {
            ...
            console.log(colorize('Invalid readiness. Using current value.', asChalkColor((asChalkColor(('red'))))));
        }
    }
    // Update the task
    ...
    console.log(colorize('✓ Task updated successfully.', asChalkColor((asChalkColor(('green'))))));
}
/**
 * Handle done action
...
async function handleDoneAction(task: any, repo: TaskRepository, results: TriageResults, options: ProcessingOptions) {
    ...
    results?.updated.push(updatedTask);
    console.log(colorize('✓ Task marked as done.', asChalkColor((asChalkColor(('green'))))));
}
/**
 * Handle tags action
...
async function handleTagsAction(task: any, repo: TaskRepository, results: TriageResults, options: ProcessingOptions) {
    const { dryRun, colorize } = options;
    if (dryRun) {
        console.log(colorize('Would update tags (dry run).', asChalkColor((asChalkColor(('yellow'))))));
        results?.updated.push({
            id: task.id,
            title: task.title,
            dry_run: true
    ...
    console.log(colorize('\nCurrent tags: ', asChalkColor((asChalkColor(('blue'))))) + (task.tags?.join(', ') || 'none'));
    const tagsInput = await new Promise<string>(resolve => {
        rl2.question(colorize('New tags (comma-separated, leave empty to keep current): ', asChalkColor((asChalkColor(('cyan'))))), resolve);
    });
    rl2.close();
    if (tagsInput.trim()) {
        ...
        results?.updated.push(updatedTask);
        console.log(colorize('✓ Tags updated successfully.', asChalkColor((asChalkColor(('green'))))));
    }
    else {
        console.log(colorize('Tags unchanged.', asChalkColor((asChalkColor(('gray'))))));
    }
}
/**
 * Handle merge action
...
async function handleMergeAction(task: any, filteredTasks: any[], repo: TaskRepository, results: TriageResults, options: ProcessingOptions) {
    const { dryRun, colorize } = options;
    if (dryRun) {
        console.log(colorize('Would merge tasks (dry run).', asChalkColor((asChalkColor(('yellow'))))));
        results?.merged.push({
            id: task.id,
            title: task.title,
            dry_run: true
    ...
    const mergeTarget = await new Promise<string>(resolve => {
        rl2.question(colorize(`Enter task number to merge with [1-${filteredTasks.length}]: `, asChalkColor((asChalkColor(('cyan'))))), resolve);
    });
    rl2.close();
    const targetIndex = parseInt(mergeTarget) - 1;
    if (isNaN(targetIndex) || targetIndex < 0 || targetIndex >= filteredTasks.length) {
        console.log(colorize('Invalid selection. Merge cancelled.', asChalkColor((asChalkColor(('red'))))));
        return;
    }
    ...
    console.log(colorize(`\nMerging:`));
    console.log(colorize(`Source: `, asChalkColor((asChalkColor(('yellow'))))) + `${task.id}: ${task.title}`);
    console.log(colorize(`Target: `, asChalkColor((asChalkColor(('green'))))) + `${targetTask.id}: ${targetTask.title}`);
    ...
    console.log(colorize('✓ Tasks merged successfully.', asChalkColor((asChalkColor(('green'))))));
}
