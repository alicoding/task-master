...
import { displayInteractiveIntro, displayEnhancedTaskDetails, displaySimilarTasksEnhanced, displayDependencies, displayActionMenu, displayHelpScreen } from './display/index';
// Import handlers
import { handleUpdateTaskAction, handleMarkAsDoneAction, handleUpdateTagsAction, handleMergeTaskAction, handleCreateSubtaskAction, handleToggleBlockedAction } from './handlers/index';
// Import prompts
 ...
 */
export async function runInteractiveMode(repo: TaskRepository, nlpService: NlpService, results: TriageResults, options: ProcessingOptions): Promise<void> {
    ...
    const pendingTasks = allTasks.filter(task => task.status !== 'done' || task.readiness === 'draft');
