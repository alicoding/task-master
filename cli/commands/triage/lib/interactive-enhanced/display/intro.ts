...
export function displayInteractiveIntro(taskCount: number, colorize: (text: string, color?: ChalkColor, style?: ChalkStyle) => string): void {
    ...
    console.log(colorize('│ INTERACTIVE TASK TRIAGE', asChalkColor((asChalkColor(('blue')))), 'bold') + colorize(' '.repeat(37) + '│', asChalkColor((asChalkColor(('blue'))))));
    console.log(colorize('└' + '─'.repeat(60) + '┘', asChalkColor((asChalkColor(('blue'))))));
    console.log(colorize(`\nFound ${taskCount} pending tasks to triage.`, asChalkColor((asChalkColor(('green'))))));
    console.log(colorize('Tasks will be presented one by one for review and action.', asChalkColor((asChalkColor(('gray'))))));
    console.log(colorize('Navigate with "n" for next and "p" for previous.', asChalkColor((asChalkColor(('gray'))))));
    console.log(colorize('Press "h" at any time to see all available commands.', asChalkColor((asChalkColor(('gray'))))));
