...
export function createColorize(useColors: boolean, jsonOutput: boolean) {
  return (text: string, color?: ChalkColor, style?: ChalkStyle) => {
    ...
    if (color) {
      ...
      switch (color) {
        case 'blue': result = chalk.blue(result); break;
    ...
    if (style) {
      switch (style) {
        case 'bold': result = chalk.bold(result); break;
        case 'italic': result = chalk.italic(result); break;
        case 'underline': result = chalk.underline(result); break;
        case 'dim': result = chalk.dim(result); break;
      }
    }
