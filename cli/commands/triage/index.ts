 ...
 */
async function processPlanFileMode(planFile: string, repo: TaskRepository, nlpService: NlpService, results: TriageResults, options: ProcessingOptions) {
 ...
 */
function handleMissingModeError(jsonOutput: boolean, colorize: (text: string, color?: ChalkColor, style?: ChalkStyle) => string, repo: TaskRepository) {
    if (jsonOutput) {
 ...
 * @param colorize Color function
 */
function outputResults(results: TriageResults, dryRun: boolean, similarityThreshold: number, jsonOutput: boolean, colorize: (text: string, color?: ChalkColor, style?: ChalkStyle) => string) {
