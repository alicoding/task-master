...
import { SearchFilters, TaskStatus, TaskReadiness, TaskOperationResult, TaskError, TaskErrorCode, isTaskStatus, isTaskReadiness, Task } from '@/core/types';
import { createNlpService } from '@/core/nlp/factory';
import { TaskSearchInfo, SimilarTask, NlpServiceInterface } from '@/core/nlp/types';
...
export class TaskSearchRepository extends BaseTaskRepository {
  ...
  private async initializeNlp(): Promise<void> {
    if (!this.nlpInitialized) {
      try {
        ...
        const { MockNlpService } = await import("@/core/nlp/services/mock-service");
  ...
  async addTask(task: Partial<Task>): Promise<Task> {
    ...
    const fullTask: Task = {
      ...
      createdAt: task.createdAt || new Date(now * 1000),
      updatedAt: task.updatedAt || new Date(now * 1000),
      parentId: task.parentId || null,
      description: task.description || null,
      body: task.body || null
    };

    // Insert task using direct SQL
    const fields = Object.keys(fullTask);
    const placeholders = fields.map(() => '?').join(', ');
    const fieldNames = fields.join(', ');
    const values = fields.map(field => {
      const value = (fullTask as any)[field];
    ...
    const sql = `INSERT INTO tasks (${fieldNames}) VALUES (${placeholders})`;
  ...
  async searchTasks(filters: SearchFilters): Promise<TaskOperationResult<Task[]>> {
    try {
      ...
      if (filters.query) {
        ...
        if (extractedFilters.query && extractedFilters.query.trim()) {
          const normalizedQuery = extractedFilters.query || '';

          // Process the query with NLP
          const processed = await this.nlpService.processQuery(normalizedQuery);

          // Create conditions for each token and stem
          const titleConditions = [
            // Original query in title or metadata
            like(tasks.title, `%${normalizedQuery}%`),
            like(tasks.metadata, `%${normalizedQuery}%`),
      ...
      // Transform array conditions into SQL WHERE clause
      if (conditions.length > 0) {
        // In a real implementation, we would convert the conditions array to SQL WHERE clauses
        ...
        // Since we're now using direct SQL, we would add conditions here
      ...
      const results = stmt.all(...queryParams) as Task[];
      return {
        success: true,
        data: results
      };
        ...
        description: (() => {
          try {
            if (typeof task.metadata === 'string') {
              ...
              return parsed && typeof parsed === 'object' && 'description' in parsed ? parsed.description : null;
            } else if (task.metadata && typeof task.metadata === 'object') {
              return 'description' in task.metadata ? (task.metadata as Record<string, any>).description : null;
            }
            return null;
          } catch (e) {
            return null;
  ...
  async naturalLanguageSearch(query: string, useFuzzy: boolean = true): Promise<TaskOperationResult<Task[]>> {
    try {
      ...
      if (extractedFilters.query && filteredTasks.length > 0) {
        const normalizedQuery = extractedFilters.query.trim() || '';
        // Map tasks for similarity search
