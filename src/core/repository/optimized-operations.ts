...
import { TaskOperationResult, TaskError, TaskErrorCode, TaskUpdateOptions, SearchFilters, Task } from '@/core/types';
...
export class OptimizedDatabaseOperations {
  ...
  async getTask(id: string): Promise<TaskOperationResult<Task>> {
    try {
      ...
      // Execute a parameterized query directly with SQLite for better type safety
      const sql = `
        SELECT * FROM tasks
        WHERE id = ?
        LIMIT 1
      ...
      const task = [stmt.get(id)] as Task[];
  ...
  async getTasks(ids: string[]): Promise<TaskOperationResult<Task[]>> {
    ...
    try {
      ...
      // For multiple IDs, use SQLite's IN operator with a parameterized query
      if (uncachedIds.length === 0) {
        return {
          success: true,
          data: cachedTasks
      ...
      const stmt = this.sqlite.prepare(sql);
      const dbTasks = stmt.all(...uncachedIds) as Task[];
  ...
  async updateTask(options: TaskUpdateOptions): Promise<TaskOperationResult<Task>> {
    try {
      ...
      // Perform the update with direct SQL for better type safety
      ...
      const updateSql = `UPDATE tasks SET ${placeholders} WHERE id = ?`;
      const updateStmt = this.sqlite.prepare(updateSql);
      ...
      // Use parameterized query to get the updated task
      const getTaskSql = `
        SELECT * FROM tasks
        WHERE id = ?
        LIMIT 1
      `;
      const getTaskStmt = this.sqlite.prepare(getTaskSql);
      const updatedTask = [getTaskStmt.get(options.id)] as Task[];
  ...
  async getAllTasks(): Promise<TaskOperationResult<Task[]>> {
    try {
      ...
      // Not in cache, query database - get all tasks
      const sql = `SELECT * FROM tasks`;
      const stmt = this.sqlite.prepare(sql);
      const taskList = stmt.all() as Task[];
  ...
  async searchTasks(filters: SearchFilters): Promise<TaskOperationResult<Task[]>> {
    try {
      ...
      // Start building our query parts
      ...
      if (filters.readiness) {
        if (Array.isArray(filters.readiness)) {
          const placeholders = filters.readiness.map(() => '?').join(',');
          if (whereClause) {
            whereClause += ` AND readiness IN (${placeholders})`;
          } else {
            whereClause = `WHERE readiness IN (${placeholders})`;
          }
          queryParams.push(...filters.readiness);
        } else {
          if (whereClause) {
            whereClause += ' AND readiness = ?';
          } else {
            whereClause = 'WHERE readiness = ?';
          }
          queryParams.push(filters.readiness);
        }
      }
