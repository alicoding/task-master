...
function formatLogMessage(level: LogLevel, message: string, context?: string): string {
  ...
  if (globalConfig.includeLevel) {
    ...
    switch (level) {
      case LogLevel.DEBUG:
        levelText = globalConfig.useColors ? chalk.blue(('DEBUG' as string)) : 'DEBUG';
        break;
      case LogLevel.INFO:
        levelText = globalConfig.useColors ? chalk.green(('INFO' as string)) : 'INFO';
        break;
      case LogLevel.WARN:
        levelText = globalConfig.useColors ? chalk.yellow(('WARN' as string)) : 'WARN';
        break;
      case LogLevel.ERROR:
        levelText = globalConfig.useColors ? chalk.red(('ERROR' as string)) : 'ERROR';
        break;
    }
