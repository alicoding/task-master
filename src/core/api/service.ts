...
import { Task, TaskWithChildren } from '@/core/types';
import {
  Operation,
  BatchOperations,
  BatchResult,
...
import { ApiService as NewApiService } from '@/core/api/service-new';
...
export class ApiService {
  ...
  private newService: NewApiService;
  ...
  constructor(dbPath: string = './db/taskmaster.db', inMemory: boolean = false, legacy: boolean = false) {
    ...
    this.newService = new NewApiService(dbPath);
  }
  ...
  async executeBatch(batch: BatchOperations, dryRun: boolean = false): Promise<BatchResult> {
    // Try using the new service
    try {
      return await this.newService.executeBatch(batch, dryRun);
    } catch (e) {
      console.warn('Failed to use new service for batch execution, falling back to legacy mode');
    }
  ...
  async exportTasks(format: string = 'json', filter?: string): Promise<ExportResult> {
    // Try using the new service
    try {
      return await this.newService.exportTasks(format, filter);
    } catch (e) {
      console.warn('Failed to use new service for export, falling back to legacy mode');
    }
  ...
  async importTasks(tasks: Task[], dryRun: boolean = false): Promise<ImportResult> {
    // Try using the new service
    try {
      return await this.newService.importTasks(tasks, dryRun);
    } catch (e) {
      console.warn('Failed to use new service for import, falling back to legacy mode');
    }
  ...
  async getTask(id: string): Promise<Task> {
    // Try using the new service
    try {
      return await this.newService.getTask(id);
    } catch (e) {
      console.warn('Failed to use new service for getTask, falling back to legacy mode');
  ...
  async getAllTasks(): Promise<Task[]> {
    // Try using the new service
    try {
      return await this.newService.getAllTasks();
    } catch (e) {
      console.warn('Failed to use new service for getAllTasks, falling back to legacy mode');
  ...
  async formatTaskHierarchy(format: string = 'text', options: any = {}): Promise<string | object> {
    // Try using the new service
    try {
      return await this.newService.formatTaskHierarchy(format, options);
    } catch (e) {
      console.warn('Failed to use new service for formatTaskHierarchy, falling back to legacy mode');
    }
