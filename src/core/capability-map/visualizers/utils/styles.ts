...
export function getColoredStatusIndicator(progress: number): string {
  if (progress >= 80) return chalk.green(('●' as string));
  if (progress >= 50) return chalk.cyan(('●' as string));
  if (progress >= 20) return chalk.yellow(('●' as string));
  return chalk.red(('●' as string));
}
