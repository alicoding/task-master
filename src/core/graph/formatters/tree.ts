...
export function formatTreeText(tasks: TaskWithChildren[], options: any = {}): string {
  ...
  if (useColor) {
    try {
      ...
      result = result
        .replace(/□/g, chalk.white(('□' as string)))
        .replace(/▶/g, chalk.yellow(('▶' as string)))
        .replace(/✓/g, chalk.green(('✓' as string)))
        .replace(/✎/g, chalk.blue(('✎' as string)))
        .replace(/▣/g, chalk.magenta(('▣' as string)))
        .replace(/⚠/g, chalk.red(('⚠' as string)));
    } catch (e) {
