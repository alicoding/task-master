...
import { formatSimpleText } from '@/core/graph/formatters/simple';
import { formatTreeText, formatHierarchyWithSymbols } from '@/core/graph/formatters/tree';
import { formatDetailedText, formatCompactText } from '@/core/graph/formatters/detailed';
import { formatEnhancedTree } from '@/core/graph/formatters/enhanced-tree';
import { formatBoxedTask } from '@/core/graph/formatters/boxed-task';
import { formatEnhancedTask } from '@/core/graph/formatters/enhanced-boxed-task';
import { formatPolishedTask } from '@/core/graph/formatters/polished-task';
import { formatTaskTable } from '@/core/graph/formatters/table-list';
import { createUiConfig, parseCliOptions, UiConfig } from '@/core/graph/formatters/ui-config';
  ...
  for (const task of tasks) {
    ...
    const tags = task.tags && task.tags.length > 0
      ? (chalk
          ? task.tags.map(tag => chalk.green(`#${tag}`)).join(' ')
          : task.tags.map(tag => `#${tag}`).join(' '))
      : (chalk ? chalk.gray(('none' as string)) : 'none');
