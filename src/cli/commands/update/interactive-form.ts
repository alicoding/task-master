...
export class InteractiveUpdateForm {
  ...
  displayBanner(): void {
    ...
    console.log(this.colorize(`📋 TASK MASTER - Update Task ${this.task.id}`, asChalkColor('blue'), asChalkColor('bold')));
    console.log(this.colorize('─'.repeat(60), asChalkColor('blue')));
    console.log('');
    console.log(this.colorize('Update the fields below (leave empty to keep current value)', asChalkColor('yellow')));
    console.log('');
  }
  ...
  private async askMultipleChoice<T extends string>(
    ...
    const choiceStr = choices.map((choice, i) => {
      ...
      const choiceText = isDefault ? this.colorize(`${choice} (current)`, asChalkColor('green')) : choice;
      ...
      this.readline.question(`${question} [${choiceStr}]: `, (answer) => {
        if (!answer.trim()) {
          ...
          if (!isNaN(num) && num >= 1 && num <= choices.length) {
            ...
            console.log(this.colorize('  Invalid choice. Using current value.', asChalkColor('red')));
            resolve(defaultChoice);
          }
        }
      });
  ...
  async run(): Promise<TaskUpdateOptions | null> {
    try {
      ...
      console.log(this.colorize('Current Task Information:', asChalkColor('cyan'), asChalkColor('bold')));
      console.log(`${this.colorize('Title:', asChalkColor('gray'))} ${this.task.title}`);
      if (this.task.description !== undefined && this.task.description !== null) {
        console.log(`${this.colorize('Description:', asChalkColor('gray'))} ${this.task.description}`);
      }
      if (this.task.body !== undefined && this.task.body !== null) {
        console.log(`${this.colorize('Body:', asChalkColor('gray'))} ${this.task.body.length > 60 ?
          this.task.body.substring(0, 60) + '...' : this.task.body}`);
      }
      console.log(`${this.colorize('Status:', asChalkColor('gray'))} ${this.task.status}`);
      console.log(`${this.colorize('Readiness:', asChalkColor('gray'))} ${this.task.readiness}`);
      console.log(`${this.colorize('Tags:', asChalkColor('gray'))} ${this.task.tags?.join(', ') || 'none'}`);
      console.log(this.colorize('\nUpdate Fields (press Enter to keep current value):', asChalkColor('yellow'), asChalkColor('bold')));
      console.log('');
      ...
      if (metadataStr && metadataStr !== currentMetadataStr) {
        try {
          ...
          console.log(this.colorize('  Invalid JSON. Metadata will not be updated.', asChalkColor('red')));
        }
      }
      ...
      if (!hasChanges) {
        console.log(this.colorize('\nNo changes were made to the task.', asChalkColor('yellow')));
        return null;
      }
      ...
      console.log('');
      console.log(this.colorize('─'.repeat(60), asChalkColor('blue')));
      console.log(this.colorize('Task Update Summary:', asChalkColor('blue'), asChalkColor('bold')));
      console.log(this.colorize('─'.repeat(60), asChalkColor('blue')));
      
      // Show what will be updated
      Object.entries(this.updateOptions).forEach(([key, value]) => {
        if (key !== 'id') {
          console.log(`${this.colorize(key + ':', asChalkColor('yellow'))} ${
      ...
      if (confirm.toLowerCase() !== 'y') {
        console.log(this.colorize('Task update cancelled.', asChalkColor('yellow')));
        return null;
      }
