...
import { ChalkColor, ChalkStyle, createColorize, asChalkColor } from '@/cli/utils/chalk-utils';
...
class SimilarTasksUI {
  ...
  displaySimilarTasks(similarTasks: any[]): void {
    console.log(this.colorize(`\nPotential duplicates found:`, asChalkColor('yellow'), asChalkColor('bold')));
    ...
    similarTasks.forEach((task, index) => {
      ...
      console.log(this.colorize(`[${selectionNumber}] ${task.id}: `, asChalkColor('blue'), asChalkColor('bold')) + task.title);
      ...
      if (score > 0.8) {
        console.log(this.colorize('   Note: This task appears to be a duplicate', asChalkColor('red')));
      }
      
      // Add separator between tasks
      if (index < similarTasks.length - 1) {
        console.log(this.colorize('   ----------------------------------------', asChalkColor('gray')));
      }
    });
  }
  ...
  async promptForAction(): Promise<string> {
    ...
    console.log(this.colorize('\nWhat would you like to do?', asChalkColor('blue'), asChalkColor('bold')));
    console.log(this.colorize('  a', asChalkColor('green')) + ') ' + this.colorize('Add anyway', asChalkColor('white')) + ' - Create a new task');
    console.log(this.colorize('  c', asChalkColor('red')) + ') ' + this.colorize('Cancel', asChalkColor('white')) + ' - Don\'t create a task');
    console.log(this.colorize('  u', asChalkColor('yellow')) + ') ' + this.colorize('Update', asChalkColor('white')) + ' - Modify an existing task instead');
    console.log(this.colorize('  m', asChalkColor('magenta')) + ') ' + this.colorize('Merge', asChalkColor('white')) + ' - Combine with an existing task');
    ...
    const answer = await new Promise<string>(resolve => {
      rl.question(this.colorize('\nChoose an option [a/c/u/m]: ', asChalkColor('cyan')), resolve);
    });
  ...
  async promptForTaskSelection(taskCount: number, operation: string): Promise<string> {
    ...
    const taskIdOrNumber = await new Promise<string>(resolve => {
      rl.question(
        this.colorize(`\nEnter task number [1-${taskCount}] or ID to ${operation}: `, asChalkColor('cyan')), 
        resolve
      );
    });
  ...
  displayMergePreview(task: any, newOptions: any, combinedTags: string[]): void {
    ...
    console.log(this.colorize('Title: ', asChalkColor('green')) + task.title + ' (unchanged)');
    console.log(this.colorize('Tags: ', asChalkColor('green')) + combinedTags.join(', '));
    console.log(this.colorize('Status: ', asChalkColor('green')) + (newOptions.status || task.status) + 
                (newOptions.status ? this.colorize(' (updated)', asChalkColor('yellow')) : ' (unchanged)'));
    console.log(this.colorize('Readiness: ', asChalkColor('green')) + (newOptions.readiness || task.readiness) + 
                (newOptions.readiness ? this.colorize(' (updated)', asChalkColor('yellow')) : ' (unchanged)'));
  }
  ...
  displayTaskResult(task: any, operation: string = 'created'): void {
    console.log(this.colorize(`\n✅ Task ${task.id} ${operation} successfully!`, asChalkColor('green'), asChalkColor('bold')));
    console.log(this.colorize(`${task.title}`, asChalkColor('white'), asChalkColor('bold')));

    if (task.description !== undefined && task.description !== null) {
      console.log(`\n${this.colorize('Description:', asChalkColor('blue'))} ${task.description}`);
    }

    if (task.body !== undefined && task.body !== null) {
      console.log(`\n${this.colorize('Details:', asChalkColor('blue'))}\n${task.body}`);
    }

    console.log(`\n${this.colorize('Status:', asChalkColor('yellow'))} ${task.status}`);
    console.log(`${this.colorize('Readiness:', asChalkColor('yellow'))} ${task.readiness}`);

    if (task.tags && task.tags.length > 0) {
      console.log(`${this.colorize('Tags:', asChalkColor('yellow'))} ${task.tags.map((tag: string) => this.colorize(`#${tag}`, asChalkColor('green'))).join(' ')}`);
    }

    if (task.parentId) {
      console.log(`${this.colorize('Parent:', asChalkColor('yellow'))} ${task.parentId}`);
    }

    console.log(`\n${this.colorize('Created:', asChalkColor('gray'))} ${new Date(task.createdAt).toLocaleString()}`);
    console.log(`${this.colorize('Updated:', asChalkColor('gray'))} ${new Date(task.updatedAt).toLocaleString()}`);

    if (task.metadata && Object.keys(task.metadata).length > 0) {
      console.log(`\n${this.colorize('Metadata:', asChalkColor('gray'))}\n${JSON.stringify(task.metadata, null, 2)}`);
    }
  }
  ...
  displayDryRunResult(title: string, noSimilarTasks: boolean = false): void {
    console.log(this.colorize('\n✅ Dry run complete - would create new task with title:', asChalkColor('green')));
    console.log(`"${title}"`);
    
    if (noSimilarTasks) {
      console.log(this.colorize('No similar tasks found above threshold.', asChalkColor('green')));
    }
  }
}
    ...
    if (filteredTasks.length > 0) {
      if (format === 'json') {
        ...
        if (!force) {
          ...
          if (response === 'c') {
            console.log(this.ui.colorize('Task creation cancelled', asChalkColor('yellow')));
            return null;
            ...
            if (isMerge) {
              ...
              console.log(this.ui.colorize(`\nMerging with task ${taskToUpdate.id}...`, asChalkColor('magenta')));
              ...
              if (!shouldProceed) {
                console.log(this.ui.colorize('Merge cancelled', asChalkColor('yellow')));
                return null;
              }
              ...
              console.log(this.ui.colorize(`\nUpdating task ${taskToUpdate.id}...`, asChalkColor('yellow')));
