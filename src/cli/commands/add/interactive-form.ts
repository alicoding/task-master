...
export class InteractiveTaskForm {
  ...
  displayBanner(): void {
    ...
    console.log(this.colorize('📋 TASK MASTER - Interactive Task Creation', asChalkColor('blue'), asChalkColor('bold')));
    console.log(this.colorize('─'.repeat(60), asChalkColor('blue')));
    console.log('');
    console.log(this.colorize('Fill out the form below to create a new task.', asChalkColor('yellow')));
    console.log(this.colorize('Required fields are marked with *', asChalkColor('red')));
    console.log('');
  }
  ...
  private async askQuestion(question: string, required: boolean = false): Promise<string> {
    const marker = required ? this.colorize('*', asChalkColor('red'), asChalkColor('bold')) + ' ' : '  ';
    
    return new Promise<string>((resolve) => {
      this.readline.question(`${marker}${question}: `, (answer) => {
        if (required && !answer.trim()) {
          console.log(this.colorize('  This field is required.', asChalkColor('red')));
  ...
  private async askMultipleChoice<T extends string>(
    ...
    const choiceStr = choices.map((choice, i) => {
      ...
      const choiceText = isDefault ? this.colorize(`${choice} (default)`, asChalkColor('green')) : choice;
      ...
      this.readline.question(`${marker}${question} [${choiceStr}]: `, (answer) => {
        if (!answer.trim()) {
          ...
          if (!isNaN(num) && num >= 1 && num <= choices.length) {
            ...
            console.log(this.colorize('  Invalid choice. Please try again.', asChalkColor('red')));
    ...
    try {
      ...
      if (metadataStr) {
        try {
          ...
          console.log(this.colorize('  Invalid JSON. Metadata will be ignored.', asChalkColor('red')));
        }
      }
      ...
      console.log('');
      console.log(this.colorize('─'.repeat(60), asChalkColor('blue')));
      console.log(this.colorize('Task Summary:', asChalkColor('blue'), asChalkColor('bold')));
      console.log(this.colorize('─'.repeat(60), asChalkColor('blue')));
      console.log(this.colorize('Title: ', asChalkColor('yellow')) + title);
      if (description) console.log(this.colorize('Description: ', asChalkColor('yellow')) + description);
      if (body) {
        console.log(this.colorize('Body: ', asChalkColor('yellow')));
        console.log(body.replace(/\\n/g, '\n'));
      }
      console.log(this.colorize('Status: ', asChalkColor('yellow')) + status);
      console.log(this.colorize('Readiness: ', asChalkColor('yellow')) + readiness);
      if (tags.length > 0) console.log(this.colorize('Tags: ', asChalkColor('yellow')) + tags.join(', '));
      if (childOf) console.log(this.colorize('Child of: ', asChalkColor('yellow')) + childOf);
      if (after) console.log(this.colorize('After: ', asChalkColor('yellow')) + after);
      console.log('');
      
      const confirm = await this.askQuestion('Create this task? (y/n)');
      if (confirm.toLowerCase() !== 'y') {
        console.log(this.colorize('Task creation cancelled.', asChalkColor('yellow')));
        return null;
      }
