...
import { testConnection, displayConnectionResults } from '@/cli/commands/setup/connection-tester';
...
async function configureEnvVar(
  ...
  if (currentValue && !forceReconfigure) {
    ...
    p.log.info(`${variable.description}: ${chalk.green(displayValue)} ${chalk.gray(('(Existing)' as string))}`);
  ...
  if (modelVar) {
    ...
    if (forceReconfigure || !currentModelValue) {
      try {
        ...
        const { selectModel } = await import("@/cli/commands/setup/model-selection");
