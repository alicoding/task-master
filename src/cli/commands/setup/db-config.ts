...
export async function setupDbConfiguration(forceReconfigure: boolean = false): Promise<void> {
  ...
  const enableMigrations = await p.confirm({
    ...
    help: 'Automatic migrations will update your database schema when Task Master is updated with new features'
  });
  ...
  const enableBackups = await p.confirm({
    ...
    help: 'Automatic backups will create a copy of your database at regular intervals'
  });
  ...
  const debugSql = await p.confirm({
    ...
    help: 'SQL debug logging will output all database queries to the console'
  });
  ...
  p.note(
    [
      ...
      `Automatic Migrations: ${finalConfig.enableMigrations ? chalk.green(('Enabled' as string)) : chalk.yellow(('Disabled' as string))}`,
      `Automatic Backups: ${finalConfig.enableBackups ? chalk.green(('Enabled' as string)) : chalk.yellow(('Disabled' as string))}`,
      finalConfig.enableBackups ? `Backup Interval: ${chalk.green(`${finalConfig.backupInterval} hours`)}` : '',
      `SQL Debug: ${finalConfig.debugSql ? chalk.green(('Enabled' as string)) : chalk.yellow(('Disabled' as string))}`
    ].filter(Boolean).join('\n'),
  ...
  const initDb = await p.confirm({
    ...
    initialValue: true,
    help: 'This will create the database and required tables'
  });
