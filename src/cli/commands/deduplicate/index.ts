...
import { createColorize, getEmptyResultsMessage, getNoTasksMessage, formatJsonOutput } from '@/cli/commands/deduplicate/lib/utils';
import { findDuplicateGroups } from '@/cli/commands/deduplicate/lib/finder';
import { displayDuplicateGroups } from '@/cli/commands/deduplicate/lib/formatter-enhanced';
import { runInteractiveMode, runAutoMergeSuggestions } from '@/cli/commands/deduplicate/lib/interactive-enhanced';
import { processTasks } from '@/cli/commands/deduplicate/lib/processor';
...
async function handleDeduplicateCommand(options: DeduplicateCommandOptions) {
  try {
    ...
    if (!jsonOutput) {
      console.log(colorize('\n🔍 Scanning for duplicate tasks...', asChalkColor('blue'), asChalkColor('bold')));
    }
    ...
    if (allTasks.length === 0) {
      console.log(colorize(getNoTasksMessage(jsonOutput), asChalkColor('yellow')));
      repo.close();
      return;
    }
    ...
    if (limitedGroups.length === 0) {
      console.log(colorize(getEmptyResultsMessage(minSimilarity, jsonOutput), asChalkColor('green')));
      repo.close();
      return;
    }
    ...
    // Exit if dry run
    if (dryRun) {
      console.log(colorize('\n✅ Dry run complete. No changes made.', asChalkColor('blue')));
      repo.close();
      return;
    }
    ...
    if (autoMerge) {
      ...
      if (highSimilarityGroups.length === 0) {
        console.log(colorize('\nNo groups with 80%+ similarity found for auto-merge.', asChalkColor('yellow')));
    ...
    console.log(colorize('\nTip: Run with --interactive for an enhanced deduplication experience!', asChalkColor('blue')));
    console.log(colorize('     Use --auto-merge to automatically handle high-similarity duplicates.', asChalkColor('blue')));
