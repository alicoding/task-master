...
import { DoDItem, DoD } from '@/core/dod/types';
  ...
  dodCommand
    ...
    .action(async (options) => {
      try {
        ...
        if (result.success) {
          console.log(chalk.green(('✅ DoD configuration initialized successfully' as string)));
          console.log(chalk.blue(('Configuration file created at:' as string)), result.data.configPath);
          console.log(chalk.blue(('Default DoD items:' as string)));
          ...
          if (result.error?.code === 'CONFIG_EXISTS' && !options.force) {
            console.log(chalk.yellow(('Use --force to overwrite existing configuration' as string)));
          }
        }
      } catch (error) {
        console.error(chalk.red(('Error initializing DoD config:' as string)), error instanceof Error ? error.message : error);
      }
    });
  ...
  dodCommand
    ...
    .action(async (item, options) => {
      try {
        ...
        if (options.task) {
          ...
          if (result.success) {
            if (options.tag) {
              ...
              console.log(chalk.green(('✅ Added DoD item to default project configuration' as string)));
            }
            console.log(`  • ${chalk.yellow(item)}`);
          } else {
            console.error(chalk.red(`❌ ${result.error?.message || 'Unknown error'}`));
        ...
        console.error(chalk.red(('Error adding DoD item:' as string)), error instanceof Error ? error.message : error);
      }
    });
  ...
  dodCommand
    ...
    .action(async (item, options) => {
      try {
        ...
        if (options.task) {
          ...
          if (result.success) {
            if (options.tag) {
              ...
              console.log(chalk.green(('✅ Removed DoD item from default project configuration' as string)));
            }
            console.log(`  • ${chalk.yellow(result.data.description || item)}`);
          } else {
            console.error(chalk.red(`❌ ${result.error?.message || 'Unknown error'}`));
        ...
        console.error(chalk.red(('Error removing DoD item:' as string)), error instanceof Error ? error.message : error);
      }
    });
  ...
  dodCommand
    ...
    .action(async (taskId, options) => {
      try {
        ...
        if (result.success) {
          if (enable) {
            ...
            if (result.data.items && result.data.items.length > 0) {
              console.log(chalk.blue(('Current DoD items:' as string)));
              result.data.items.forEach(item => {
                const status = item.completed ? 
                  chalk.green(('✓' as string)) : 
                  chalk.yellow(('☐' as string));
                console.log(`  ${status} ${item.description}`);
              });
            } else {
              console.log(chalk.yellow(('No DoD items defined for this task yet.' as string)));
        ...
        console.error(chalk.red(('Error toggling DoD:' as string)), error instanceof Error ? error.message : error);
      }
    });
  ...
  dodCommand
    ...
    .action(async (options) => {
      try {
        ...
        if (options.task) {
          ...
          if (result.success) {
            ...
            if (result.data.enabled === false) {
              console.log(chalk.yellow(('⚠️ DoD is disabled for this task' as string)));
              return;
            }
            
            if (!result.data.items || result.data.items.length === 0) {
              console.log(chalk.yellow(('No DoD items defined for this task.' as string)));
              return;
            }
          ...
          if (result.success) {
            if (options.tag && result.data.tagItems && result.data.tagItems[options.tag]) {
              ...
              console.log(chalk.blue(('Default Definition of Done:' as string)));
              
              if (!result.data.defaultItems || result.data.defaultItems.length === 0) {
                console.log(chalk.yellow(('No default DoD items defined.' as string)));
              } else {
                result.data.defaultItems.forEach(item => {
                  console.log(`  • ${chalk.yellow(item.description)}`);
                });
              ...
              if (result.data.tagItems && Object.keys(result.data.tagItems).length > 0) {
                console.log('');
                console.log(chalk.blue(('Tag-specific Definitions of Done:' as string)));
        ...
        console.error(chalk.red(('Error listing DoD items:' as string)), error instanceof Error ? error.message : error);
      }
    });
    ...
    .action(async (taskId) => {
      try {
        ...
        if (result.success) {
          ...
          if (completed === total) {
            console.log(chalk.green(('✅ All DoD items are complete!' as string)));
          } else {
        ...
        console.error(chalk.red(('Error checking DoD status:' as string)), error instanceof Error ? error.message : error);
      }
    });
  ...
  dodCommand
    ...
    .action(async (taskId, itemId, options) => {
      try {
        ...
        console.error(chalk.red(('Error marking DoD item:' as string)), error instanceof Error ? error.message : error);
      }
    });
