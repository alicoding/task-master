...
export async function handleAutoMerge(
  ...
  if (score >= 0.8) {
    if (!jsonOutput) {
      console.log(colorize(`│    🔄 Auto-merging with ${bestMatch.id} (${percentage}% similarity)`, asChalkColor('magenta')));
    }
    ...
    if (!dryRun) {
      ...
      if (!jsonOutput) {
        console.log(colorize(`│    ✓ Merged successfully with task ${bestMatch.id}`, asChalkColor('green')));
        
        // Show what was merged
        if (combinedTags.length > bestMatch.tags.length) {
          const newTags = combinedTags.filter(t => !bestMatch.tags.includes(t));
          console.log(colorize(`│      Added tags: ${newTags.join(', ')}`, asChalkColor('cyan')));
        }
      }
    } else {
      results.merged.push({
      ...
      if (!jsonOutput) {
        console.log(colorize(`│    ✓ Would auto-merge with task ${bestMatch.id}`, asChalkColor('magenta')));
      }
    }
  } else {
    // Similarity not high enough for auto-merge, create new task
    if (!jsonOutput) {
      console.log(colorize(`│    ℹ Similarity below threshold for auto-merge (${percentage}% < 80%)`, asChalkColor('blue')));
    }
