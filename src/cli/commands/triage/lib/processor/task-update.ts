...
export async function handleTaskUpdate(
  ...
  if (!dryRun) {
    ...
    if (!jsonOutput) {
      ...
      console.log(colorize(`│    ✓ Updated task ${updatedTask.id}`, asChalkColor('yellow')));
      
      if (changes.length > 0) {
        console.log(colorize(`│      Changed: ${changes.join(', ')}`, asChalkColor('gray')));
      }
    }
  } else {
    const simTask = {
    ...
    if (!jsonOutput) {
      console.log(colorize(`│    ✓ Would update task ${simTask.id}`, asChalkColor('yellow')));
    }
  }
}