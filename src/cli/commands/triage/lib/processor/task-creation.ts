...
export async function createNewTask(
  ...
  if (!dryRun) {
    ...
    if (!jsonOutput) {
      console.log(colorize(`│    ✓ Created new task ${newTask.id}`, asChalkColor('green')));
      
      // Show hierarchy info if applicable
      if (newTask.parentId) {
        console.log(colorize(`│      Child of task: ${newTask.parentId}`, asChalkColor('blue')));
      }
      ...
      if (properties.length > 0) {
        console.log(colorize(`│      Properties: ${properties.join(', ')}`, asChalkColor('gray')));
      }
    }
  } else {
    const simTask = {
    ...
    if (!jsonOutput) {
      console.log(colorize(`│    ✓ Would create new task: "${taskData.title}"`, asChalkColor('green')));
    }
  }
}