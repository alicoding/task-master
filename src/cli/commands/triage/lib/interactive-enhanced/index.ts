...
import { ProcessingOptions, TriageResults } from '@/cli/commands/triage/lib/utils';
import { sortPendingTasks } from '@/cli/commands/triage/lib/interactive-enhanced/utils/sorting';
...
import {
  ...
  displayActionMenu,
  displayHelpScreen
} from '@/cli/commands/triage/lib/interactive-enhanced/display/index';
...
} from '@/cli/commands/triage/lib/interactive-enhanced/handlers/index';

// Import prompts
import { promptForAction } from '@/cli/commands/triage/lib/interactive-enhanced/prompts/index';
...
export async function runInteractiveMode(
  ...
  if (pendingTasks.length === 0) {
    if (jsonOutput) {
      ...
      console.log(colorize('No pending tasks to triage.', asChalkColor('yellow')));
    }
    return;
  }
  ...
  while (running && taskIndex < sortedTasks.length) {
    ...
    if (jsonOutput) {
      ...
      switch (action) {
        case 'q': // Quit
          console.log(colorize('Exiting triage mode.', asChalkColor('yellow')));
          running = false;
          break;
        ...
        case 'm': // Merge (if similar tasks exist)
          if (filteredTasks.length > 0) {
            ...
            console.log(colorize('No similar tasks available for merging.', asChalkColor('yellow')));
          }
          taskIndex++;
          break;
        ...
        default:
          console.log(colorize('Invalid option. Press h for help.', asChalkColor('red')));
          break;
      }
    }
  }
  ...
  if (!jsonOutput && running) {
    console.log(colorize('\n✅ All pending tasks have been triaged!', asChalkColor('green'), asChalkColor('bold')));
  }
}