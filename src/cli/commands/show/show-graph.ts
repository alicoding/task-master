...
export async function createShowGraphCommand() {
  const showGraphCommand = new Command('graph')
    ...
    .option('--unicode', (asChalkColor('Use unicode box drawing characters')))
    .option('--no-unicode', (asChalkColor('Disable unicode box drawing characters')))
  ...
  const helpFormatterModule = await import("@/cli/helpers/help-formatter");
