...
import { OutputFormat, Task } from '@/core/types';
import type { HierarchyTask } from '@/core/repository/hierarchy';
import { createShowGraphCommand } from '@/cli/commands/show/show-graph';
import { createDepsCommand } from '@/cli/commands/show/deps';
...
export async function createShowCommand() {
  const showCommand = new Command('show')
    ...
    .option('--boxes', (asChalkColor('Use box drawing for task sections')))
    .option('--no-boxes', (asChalkColor('Disable box drawing for task sections')))
    .option('--tables', (asChalkColor('Use formatted tables for task lists')))
    .option('--no-tables', (asChalkColor('Disable formatted tables for task lists')))
    .option('--compact', (asChalkColor('Use compact display format')))
    .option('--compatibility-mode', (asChalkColor('Use compatibility mode for limited terminals')))
    .option('--show-metadata', (asChalkColor('Include metadata in the output')))
    .option('--hide-description', (asChalkColor('Hide description in the output')))
    .option('--full-content', (asChalkColor('Show all content without truncation for long descriptions and body text')))
