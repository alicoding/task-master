...
import { getColorFunctions } from '@/cli/commands/search/color-utils';
...
function displaySearchExplanation(
  ...
  console.log(colorize('Search query analysis:', asChalkColor('blue'), asChalkColor('bold')));
  console.log(colorize(`Original query: "${query}"`, asChalkColor('green')));
  
  if (extractedInfo.extractedTerms.length > 0) {
    console.log(colorize('Extracted filters:', asChalkColor('blue')));
    for (const term of extractedInfo.extractedTerms) {
      console.log(`  - ${colorize(term, asChalkColor('magenta'))}`);
    }
  }
  
  if (extractedInfo.query !== query) {
    console.log(colorize('Cleaned query: ', asChalkColor('blue')) + 
                colorize(`"${extractedInfo.query}"`, asChalkColor('green')));
  }
  
  console.log(colorize(`Fuzzy matching: ${useFuzzy ? 'enabled' : 'disabled'}`, asChalkColor('blue')));
...
function displayTaskDetails(task: Task, useColor?: boolean): void {
  ...
  const metadata = typeof task.metadata === 'string' 
    ? (((JSON.parse(task.metadata) as Partial<TaskMetadata>) as Partial<TaskMetadata>) as Partial<TaskMetadata>) 
    : task.metadata;
...
export async function performSimilaritySearch(
  ...
  similarTasks.forEach(task => {
    ...
    console.log(`  ${colorize('Similarity: ', asChalkColor('blue'))}${colorize(`${percentage}%`, barColor)} ${colorize(bar, barColor)}`);
    console.log(`  Tags: ${task.tags?.join(', ') || 'none'}`);
