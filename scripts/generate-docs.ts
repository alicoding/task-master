...
// Files to exclude
const EXCLUDE_FILES = ['scripts/generate-docs.js', 'scripts/generate-docs.ts'];
 ...
 */
async function extractDocs(files: string[]): Promise<DocsStructure> {
  const docs: DocsStructure = {};
 ...
 */
function generateDocContent(docs: DocsStructure): string {
 ...
 */
async function updateDocFile(filePath: string, content: string): Promise<void> {
  try {
