...
// Type for database column information
...
async function fixDatabase(): Promise<void> {
  ...
  try {
    ...
    try {
      const tableInfo = db.prepare("PRAGMA table_info(tasks)").all() as ColumnInfo[];
    ...
    if (!hasDescription || !hasBody) {
      ...
      try {
        ...
        const updatedTableInfo = db.prepare("PRAGMA table_info(tasks)").all() as ColumnInfo[];
        ...
        const error = err as Error;
        console.error(`${colors.red}Error adding columns: ${error.message}${colors.reset}`);
        throw err;
      }
    } else {
